...
with open("/local_data2/sung/dataset/sitcom_reasoning_val_ablation.json", "r") as f:
    sitcom_detection_val_a = json.load(f)


model_path = "/local_data2/sung/checkpoints/sitcom_detection_0530_1/checkpoint-40"

model, tokenizer = load_model(
        model_path, "cuda", 1, None, False, False, False)

model_path_2 = "/local_data2/sung/checkpoints/sitcom_detection_0530_1_ablation/checkpoint-40"

model_a, tokenizer_a = load_model(
        model_path_2, "cuda", 1, None, False, False, False)

# # sitcom reasoning eval
# # evaluation
conv = get_default_conv_template(model_path).copy()
...
txt_path = "/local_data2/sung/sitcom.txt"
ted_f = open(txt_path, "w")

for i in tqdm(range(len(sitcom_detection_val))):
    # if i==10:
    #     continue

    ted_f.write(sitcom_detection_val[i]["id"]+"\n")
    ted_f.write("[GT]:  " + sitcom_detection_val[i]['conversations'][1]['value'] + "\n\n")
    ...
    output_ids_ = output_ids[0][len(input_ids[0]):]
    outputs = tokenizer.decode(output_ids_, skip_special_tokens=True).strip()
    ted_f.write("[OURS]:  " + outputs+"\n\n")



    inputs_ = sitcom_detection_val_a[i]['conversations'][0]['value'] + "### Assistant:"

    input_ids_ = tokenizer_a([inputs_]).input_ids
    with torch.no_grad():
        output_ids_ = model_a.generate(
            torch.as_tensor(input_ids_).cuda(),
            do_sample=True,
            temperature=0.5,
            max_new_tokens=5000,
        )

    output_ids__ = output_ids_[0][len(input_ids_[0]):]
    outputs_ = tokenizer_a.decode(output_ids__, skip_special_tokens=True).strip()
    ted_f.write("[ABLA]:  " + outputs_ + "\n\n\n\n")
