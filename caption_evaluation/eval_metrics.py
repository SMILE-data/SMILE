...
import sys
sys.path.append("/home/sung/krafton/Conversation/bartscore")
...
def evaluate_metrics_total(prediction_file: Union[str, Path, List[Dict[str, str]]],
    ...
    gt_txt = open(gt_path,"w")
    for jj in range(len(predictions)):
        pred_txt.write(predictions[jj]+"\n")
        gt_txt.write(ground_truths[jj][0]+"\n")
    pred_txt.close()
    gt_txt.close()

    termianl_command=f"bert-score -r {pred_path} -c {gt_path}  --lang en --rescale_with_baseline"

    os.system(termianl_command)
    os.remove(pred_path)
    os.remove(gt_path)
